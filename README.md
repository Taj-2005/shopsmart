...
| **ShopSmart Docker Compose Up (EC2)** | `shopsmart_docker_compose_up_EC2.yml` | Manual only (`workflow_dispatch`) | SSHs to EC2, updates the repo at `SHOPSMART_DEPLOY_PATH` (default `/opt/shopsmart`), writes `shopsmart/server/.env` from `SHOPSMART_SERVER_ENV`, sets `NEXT_PUBLIC_API_URL` for the client build (input or `http://<EC2_HOST>:4000`), stops **pm2** `shopsmart-api` and conflicting containers, then runs `docker compose build` + `docker compose up -d` in `shopsmart/`. Open security group ports **3000** (client) and **4000** (API). Set `FRONTEND_URL` in the server env to your public client URL (e.g. `http://<EC2_HOST>:3000`) for CORS. |
